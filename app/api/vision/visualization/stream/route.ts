...
/**
 * GET - MJPEG stream from ROS 2 visualization (with YOLO overlays)
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BRIDGE_URL}/api/vision/visualization/stream`, {
      cache: "no-store",
      next: { revalidate: 0 },
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Visualization stream not available" },
    ...
    console.error("Visualization stream proxy error:", error);
    return NextResponse.json(
      { error: "Failed to fetch visualization stream" },
      { status: 500 }
    );
  }
}
